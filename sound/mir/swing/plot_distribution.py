...
def plot_mode_distribution(indir, outfile):
    ...
    for infile in glob.glob(os.path.join(indir, '*.mid')):
        ...
        try:
            ...
            if key not in mode_hist.keys():
                mode_hist.update({key: 1})
            else:
                key_count = mode_hist.get(key)
                key_count += 1
                mode_hist.update({key: key_count})
    ...
    # Histogram
    size = len(mode_hist.items())
    plt.bar(np.arange(size), mode_hist.values());
    plt.xticks(np.arange(size), mode_hist.keys(), rotation='vertical')
    plt.xlabel('Mode')
    plt.ylabel('Proportion')
    plt.subplots_adjust(bottom=0.15)
...
def plot_meter_distribution(indir, outfile):
    ...
    for infile in glob.glob(os.path.join(indir, '*.mid')):
        ...
        try:
            ...
            if meter not in meter_hist.keys():
                meter_hist.update({meter: 1})
            else:
                meter_count = meter_hist.get(meter)
                meter_count += 1
                meter_hist.update({meter: meter_count})
    ...
    # Histogram
    size = len(meter_hist.items())
    plt.bar(np.arange(size), meter_hist.values());
    plt.xticks(np.arange(size), meter_hist.keys())
    plt.xlabel('Meter')
    plt.ylabel('Proportion')
    plt.subplots_adjust(bottom=0.15)
...
if __name__ == '__main__':
    ...
    # Original mode
    outdir = os.path.join('midis_final_selection', 'swing_pianos')
    ...
    outfile_mode = os.path.join('plots', 'swing_pianos_mode_distribution.png')
    outfile_meter = os.path.join('plots', 'swing_pianos_meter_distribution.png')
    ...
    # plot_mode_distribution(indir=outdir, outfile=outfile_mode)

    # Step 5 - Plot the meter distribution
    plot_meter_distribution(indir=outdir, outfile=outfile_meter)
